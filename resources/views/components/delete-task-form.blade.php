...
<form id="deleteTaskModal" style="display: none;"
    ...
    <div class="bg-component p-6 rounded-lg shadow-lg md:w-5/12 w-8/12 lg:mt-16 md:max-h-[90vh] overflow-y-auto z-30">
        ...
        <x-input-label class="text-xl">{{ __('Are you sure you want to delete') }} <span class="font-bold"></span>?</x-input-label>
        <input type="hidden" id="task_id" name="task_id"/>

        <div class="flex justify-end gap-4">
            <x-primary-button type="button" id="cancelDeleteTask">{{ __('Cancel') }}</x-primary-button>
